...
function char_val() {
  ...
  const str = ret[2].join("");
  return new ast.CaseInsensitiveString(str, "quote");
}
...
function parseString(rules, input, opts = {}) {
  if (!opts.strict) {
...
function parseStream(rules, input, opts) {
...
exports.parseFile = function parseFile(input, opts) {
  const r = new ast.Rules();
  return parseStream(r, fs.createReadStream(input), opts);
};
